...
流程结束回调地址使用 `callback.url`；节点完成回调和驳回通知优先使用 slotConfig 中的节点级 `callbackUrl`，为空时降级使用 `callback.url`。
...
**驳回通知**：驳回跳转和审计记录写入完成后，流程中心主动发送 `REJECT_OCCURRED` 通知。URL 解析顺序与节点完成回调一致：当前驳回节点的 `callbackUrl` 优先，未配置时降级到流程级 `callback.url`；发送失败只记录日志，不影响驳回接口返回。
...
`callbackType` 当前支持三种 Flowable HTTP ServiceTask 触发类型：
...
**预期**：流程中心按 `callbackNodeKey` 查找 `NodeSemanticInfo.callbackUrl` 并发送一次 POST；节点级 URL 为空时降级到 ES `ProcessMetadataDocument.callback.url`。ES `status` 不变（仍为 `running`），返回 200。

**URL 解析优先级**：

```text
NodeSemanticInfo.CallbackUrl
...
**预期**：缺少 `callbackNodeKey` 时返回 200 并跳过通知；若 `callbackNodeKey` 存在但节点级 `callbackUrl` 和流程级 `callback.url` 均为空，也返回 200 并跳过通知。

#### 场景 D：驳回通知（流程中心 → 业务系统）

驳回通知不经过 Flowable HTTP ServiceTask。`/api/tasks/complete` 驳回成功后，流程中心复用节点级回调 payload 主动发送：
...
```json
{
  ...
  "lastAuditRecord": {
    ...
    "comment": "退回修改",
    "rejectReason": "材料不完整",
    "operatedAt": "2026-05-24T02:33:54.7587074Z",
    "slotSelections": []
  },
  "triggeredAt": "2026-05-24T02:33:55.1000000Z"
}
```

**预期**：业务系统按 `callbackType=REJECT_OCCURRED` 识别驳回事件，并通过 `rejectTargetNodeKey` 判断流程退回目标节点。

#### 场景 E：幂等测试（已是 completed 状态重复回调）
...
| 流程结束通知业务系统失败（非 2xx） | 500（触发 Flowable 重试） |

> 节点完成回调和驳回通知发送失败时，只记录 Error 日志，不影响流程推进或驳回结果。
...
| T-CB-05 | 节点级 `callbackUrl` 和流程级 `callback.url` 均为空 | 返回 200，业务系统不收到通知 |
| T-CB-06 | 业务系统回调返回 500 | 流程中心返回 200（不重试，Phase 1 限制），日志有 Error |
| T-CB-07 | 无 `callbackType`（原有流程结束回调） | 走原有路径，ES status=completed，行为不变 |
| T-CB-08 | 驳回成功 | 业务系统收到 `REJECT_OCCURRED`，包含 `rejectTargetNodeKey` |
...
在需要触发节点完成回调的节点后，在 BPMN 中追加一个 HTTP ServiceTask。BPMN 只负责通知流程中心节点已完成，不再携带业务系统 URL：
...
<serviceTask id="st_notify_integrity_head_handle_completed"
  ...
  <extensionElements>
    ...
    <flowable:field name="requestBody">
      <flowable:expression>{"processInstanceId":"${execution.processInstanceId}","businessId":"${businessId}","processDefinitionKey":"${processDefinitionKey}","variables":{"callbackType":"NODE_COMPLETED","callbackNodeKey":"ut04_integrity_head_handle"}}</flowable:expression>
    </flowable:field>
...
节点级业务回调地址配置在 slotConfig：

```json
{
  "taskDefinitionKey": "ut04_integrity_head_handle",
  "nodeSemantic": "INTEGRITY_HEAD_HANDLE",
  "callbackUrl": "https://biz-system.internal/api/node-callback",
  "slots": []
}
```

`callbackUrl` 为空或缺省时，流程中心降级使用启动流程时传入的 `callback.url`；两者均为空时跳过节点回调。
...
| 节点 | callbackType | 结果 |
|---|---|---|
| `ut04_integrity_head_handle` | `NODE_COMPLETED` | `callbackUrl` 收到 POST，返回 200 |
| `ut08_secretary_final_approve` | `NODE_COMPLETED` | `callbackUrl` 收到 POST，返回 200 |
...
| 回调地址治理 | `callbackUrl` / `callback.url` 不允许任意公网地址，生产环境需白名单或从业务配置读取 |
...
| `未配置回调 URL` | Debug | 节点级和流程级回调地址均为空，已跳过 |
